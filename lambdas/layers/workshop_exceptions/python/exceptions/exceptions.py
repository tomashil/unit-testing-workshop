...
class CustomResourceResponseError(WorkshopError):
    """Exception raised for failure related to 'PUT' request from Lambda custom resource to CloudFormation
    response S3 bucket."""
    def __init__(self, error_code, response):
        """Default constructor.
        
        Args:
            error_code: HTTP error code at time of failure
            response: JSON-formatted response object received at time of failure
        """
        self.message = f"{self.__class__.__name__}: {error_code} error raised when attempting to send response from custom resource: \n{response}"
...
class S3DeleteObjectError(WorkshopError):
    """Exception raised for error occurring during clean up of S3 bucket in custom resource 'DELETE' operation."""
    def __init__(self, client_error_code, message):
        """Default constructor.
